#!/bin/bash
# -------------------------------------------------------------------------------
# Filename:    4_local_loop_add_remove_lun.sh  
# Revision:    1.0
# Date:        2018/08/11
# Author:      yangyang
...
$cli mapping-group remove block-volume  ${mp_id[1]} ${vol_id2}
$cli mapping-group remove block-volume  ${mp_id[2]} ${vol_id3}
sleep 180
}
...
for i in {1..10}
do
echo "add lun to mapping-group"
add_mp_lun
sleep 10
echo "remove lun to mapping-group"
remove_mp_lun
done
}
