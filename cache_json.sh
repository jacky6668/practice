        ...
        do
            ...
            do
                ...
                do
                    for ip in ${host}
                    do
                        ssh root@${ip} "/root/testtools/ebs/set_flush_water_level.sh 99" &
                    done # end of ip
                    sleep 15
                    for ip in ${host}
                    do
                        ssh root@${ip} "fio -thread -ioengine=libaio -group_reporting -name=mytest -randrepeat=0 -time_based -runtime=600 -direct=1 -filename=/dev/sdi:/dev/sdj -rw=randwrite -bs=4k -numjob=8 --iodepth=256" &
                    done # end of ip
                    sleep 630 
                    for ip in ${host}
                    do
                        ssh root@${ip} "/root/testtools/ebs/set_flush_water_level.sh 60" &
                    done # end of ip
                    sleep 15
                    for ip in ${host}
                    do
